...
firstError()
{
  awk '/^Parser error in .*[Ll]ine *[0-9]*/ { errors++; if (errors>1) exit }\
                              { if (errors) print }' $1
}
...
mydiff()
{
  ...
  i=0
  printf "%15s  ------------------------------------------------------------\n" " "
  while read file fname line lnum word1 word2 word3; do
    oracle+=("$file $fname $line $lnum $word1 $word2 $word3")
    printf "%15s  | %s\n" " " "${oracle[$i]}"
    i=$[i+1]
  done < $1
  printf "%15s  ------------------------------------------------------------\n" " "
  ...
  i=0
  while read sfile sfn sline sln sword1 sword2 sword3; do
    read ofile ofn oline oln oword1 oword2 oword3 <<< "${oracle[$i]}"
    ...
    highlight "$oln" "$sln"
    highlight "$oword1" "$sword1"
    highlight "$oword2" "$sword2"
    highlight "$oword3" "$sword3"
    printf "\n"
...
timeoutCompile()
{
  ...
  nice $EXE -2 $inbase.c 1> /dev/null 2> $inbase.$ERR &
  local pid=$!
